...
models_directories = ["MODELS_SIMPLE/INVERSE_REPTILE_ENEMIES", "MODELS_SIMPLE/INVERSE_REPTILE_DOOR", "MODELS_SIMPLE/INVERSE_REPTILE_KEY"]
...
for task, lvl_name, models_dir, llave in zip(tasks, lvl_names, models_directories, has_key):
    ...
    for mapa, i, pos in zip(mapas, range(len(mapas)), posiciones):
        ...
        model.learn(total_timesteps=TIMESTEPS, reset_num_timesteps=False, tb_log_name=f"INVERSE_REPTILE_{task.name}")
...
if not os.path.exists("MODELS_SIMPLE/simple_reptile_inverse"):
    os.makedirs("MODELS_SIMPLE/simple_reptile_inverse")

model.save("MODELS_SIMPLE/simple_reptile_inverse/model_final")


# Evaluar el modelo en cada uno de los mapas
zelda_levels = ["zelda_0.txt", "zelda_1.txt", "zelda_2.txt", "zelda_3.txt", "zelda_4.txt"]
evaluar_tarea("MODELS_SIMPLE/simple_reptile_inverse/model_final.zip", "inverse_simple_reptile", csv_dir, "REPTILE_INVERSE", zelda_levels, posiciones, Task.ZELDA)
