...
models_dir = "MODELS_CL/CL_R_BETA_ORDER"
logs_dir    = "LOGS_CL/CL_R_BETA_ORDER"
csv_dir = "CSV"
...
model = sb3.A2C.load("MODELS_SIMPLE/simple_r_Beta_order/model_final.zip", env=env, verbose=1, tensorboard_log=f"{logs_dir}")
...
for mapa, i, pos in zip(mapas, range(len(mapas)), posiciones):
    ...
    model.set_env(env)
    model.learn(total_timesteps=TIMESTEPS, reset_num_timesteps=False, tb_log_name=f"CL_R_BETA_ORDER")
    model.save(f"{models_dir}/model_{i}")

# Evaluar el modelo en cada uno de los mapas
model_path = f"{models_dir}/model_{len(mapas) - 1}.zip"
evaluar_tarea(model_path, "cl_r_Beta_order", csv_dir, "CL_R_BETA_ORDER", level_files, posiciones, TASK)
