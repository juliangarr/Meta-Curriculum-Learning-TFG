...
models_dir = "MODELS_CL/INVERSE_CL_REPTILE"
logs_dir    = "LOGS_CL/INVERSE_CL_REPTILE"
csv_dir = "CSV"
...
model = sb3.A2C.load("MODELS_SIMPLE/simple_reptile_inverse/model_final.zip", env=env, verbose=1, tensorboard_log=f"{logs_dir}")
...
for mapa, i, pos in zip(mapas, range(len(mapas)), posiciones):
    ...
    model.set_env(env)
    model.learn(total_timesteps=TIMESTEPS, reset_num_timesteps=False, tb_log_name=f"INVERSE_CL_REPTILE")
    model.save(f"{models_dir}/model_{i}")

# Evaluar el modelo en cada uno de los mapas
model_path = f"{models_dir}/model_{len(mapas) - 1}.zip"
evaluar_tarea(model_path, "cl_inverse_reptile", csv_dir, "CL_INVERSE_REPTILE", level_files, posiciones, TASK)
