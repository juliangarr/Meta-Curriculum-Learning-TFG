...
model = sb3.A2C.load("MODELS_SIMPLE/simple/model_final.zip", env=env, verbose=1, tensorboard_log=f"{logs_dir}")
...
for mapa, i, pos in zip(mapas, range(len(mapas)), posiciones):
    ...
    model.set_env(env)
    model.learn(total_timesteps=TIMESTEPS, reset_num_timesteps=False, tb_log_name=f"CL")
...
evaluar_tarea(model_path, csv_filename, csv_dir, "CL", level_files, posiciones, TASK)