...
models_dir = "MODELS_CL/INVERSE_CL"
logs_dir    = "LOGS_CL/INVERSE_CL"
csv_dir = "CSV"
...
model = sb3.A2C.load("MODELS_SIMPLE/inverse/model_final.zip", env=env, verbose=1, tensorboard_log=f"{logs_dir}")
...
for mapa, i, pos in zip(mapas, range(len(mapas)), posiciones):
    ...
    model.set_env(env)
    model.learn(total_timesteps=TIMESTEPS, reset_num_timesteps=False, tb_log_name=f"INVERSE_CL")
...
csv_filename = "cl_inverse"

evaluar_tarea(model_path, csv_filename, csv_dir, "CL_INVERSE", level_files, posiciones, TASK)