...
models_dir = "MODELS_SIMPLE/R_ORDER"
logs_dir_inicial = "LOGS_SIMPLE/R_ORDER"
...
for i, pos  in zip(range(len(posiciones)), posiciones):
    ...
    for task, mapa, llave in zip(tasks, mapas, has_key):
        ...
        model.learn(total_timesteps=TIMESTEPS, reset_num_timesteps=False, tb_log_name=f"R_ORDER_{i}_{task.name}")
...
if not os.path.exists("MODELS_SIMPLE/simple_r_order"):
    os.makedirs("MODELS_SIMPLE/simple_r_order")

model.save("MODELS_SIMPLE/simple_r_order/model_final")


# Evaluar el modelo en cada uno de los mapas
zelda_levels = ["zelda_0.txt", "zelda_1.txt", "zelda_2.txt", "zelda_3.txt", "zelda_4.txt"]
evaluar_tarea("MODELS_SIMPLE/simple_r_order/model_final.zip", "simple_r_order", csv_dir, "R_ORDER", zelda_levels, posiciones, Task.ZELDA)
